    ...
    public class CommandBoard : Form
    {
        ...
        private void InitializeControls()
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                descriptionTextBoxes[i].Paint += (s, e) =>
                {
                    TextBox tb = (TextBox)s;
                    using (Pen pen = new Pen(ControlBorderColor, 2))
                    {
            ...
            pageDescriptionTextBox.Paint += (s, e) =>
            {
                TextBox tb = (TextBox)s;
                using (Pen pen = new Pen(ControlBorderColor, 2))
                {
        ...
        private void LayoutControls()
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                descriptionTextBoxes[i].Location = new Point(
                    startX + 30 + totalGridWidth + buttonSpacing + labelWidth + commandWidth + commandSpacing,
                    startY + i * textBoxHeight
                );
...
Panel leftPanel = new Panel
{
    ...
    AutoScroll = true,
    BorderStyle = BorderStyle.FixedSingle // Optional: for visual clarity
};
int buttonHeight = 30;
int buttonSpacing = 35;
int totalContentHeight = variableNames.Length * buttonSpacing + 10; // Calculate total height needed
...
// Set the AutoScrollMinSize to ensure the panel knows the full content size
...
private void UpdateSelectedColor()
{
    ...
    Color adjustedColor = Color.FromArgb(r, g, b); // Changed FromRGB to FromArgb
